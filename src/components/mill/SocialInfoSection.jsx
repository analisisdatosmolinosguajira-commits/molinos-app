...
const SocialInfoSection = ({ socialData, onRefresh }) => {
    ...
    if (socialData.hasNoCommunity || !socialData.community) {
        return (
            <div className="bg-white rounded-xl border border-slate-200 p-6">
                <div className="flex items-center gap-3 text-amber-600 mb-3">
                    <AlertCircle size={24} />
                    <h3 className="text-lg font-bold">Sin Comunidad Asignada</h3>
                </div>
                <p className="text-slate-600">
                    Este molino no tiene una comunidad asignada actualmente.
                </p>
            </div>
        );
    }

    const { community, status, concertation_date, concertations, members } = socialData;
    ...
    return (
        <>
            <div className="space-y-6">
                ...
                {/* Active Social Situations Alert */}
                {socialData.activeSituations && socialData.activeSituations.length > 0 && (
                    <div className="bg-gradient-to-r from-amber-50 to-red-50 rounded-xl border-2 border-amber-300 p-6">
                        <div className="flex items-start gap-3 mb-4">
                            <AlertCircle size={24} className="text-amber-600 flex-shrink-0 mt-0.5" />
                            <div>
                                <h4 className="font-bold text-amber-900 text-lg">
                                    Situaciones Sociales Activas ({socialData.activeSituations.length})
                                </h4>
