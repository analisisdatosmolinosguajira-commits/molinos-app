...
-- Script estricto para poblar tablas complementarias de movimientos existentes
-- Se eliminaron lost 'ON CONFLICT DO NOTHING' para que reporten error si hay duplicados.

-- 1. COMMUNITIES (Destinos)
...
-- 2. PERSONNEL (Conductores y Cuadrillas)
-- Asignar Conductor 'Luis'
...
WHERE (m.vehicle_info IS DISTINCT FROM 'Nissan Frontier - Placa CMD-123')
  ...
  -- Evitar duplicados explicitamente para que el script sea re-ejecutable sin error de unique si ya existe
  AND NOT EXISTS (SELECT 1 FROM movement_person mp WHERE mp.movement_id = m.movement_id AND mp.role = 'Conductor');

-- Asignar Tecnico 'Maria'
...
WHERE m.objective = 'diagnostico'
  ...
  AND NOT EXISTS (SELECT 1 FROM movement_person mp WHERE mp.movement_id = m.movement_id AND mp.crew_id IS NOT NULL);
...
WHERE m.objective = 'concertacion'
  ...
  AND NOT EXISTS (SELECT 1 FROM movement_person mp WHERE mp.movement_id = m.movement_id AND mp.person_id = p.person_id);
...
WHERE m.objective = 'inspeccion'
  ...
  AND NOT EXISTS (SELECT 1 FROM movement_work_order mw WHERE mw.movement_id = m.movement_id)
LIMIT 5;
...
WHERE m.objective = 'diagnostico'
  AND (m.vehicle_info IS DISTINCT FROM 'Nissan Frontier - Placa CMD-123')
  AND NOT EXISTS (SELECT 1 FROM movement_diagnosis md WHERE md.movement_id = m.movement_id)
LIMIT 5;
